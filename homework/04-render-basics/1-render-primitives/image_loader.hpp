...
int
load (std::istream &src, canvas<compT> &img)
{
  if (!src.good ())
    return EXIT_FAILURE;
  ...
  sep = src.get ();
  if ((fmt != "P6" and fmt != "P3") or width > limits::MAX_IMAGE_SIZE
      or height > limits::MAX_IMAGE_SIZE
      or max_color_val > limits::MAX_COLOR_VALUE
      or ((1U << 8 * sizeof (compT)) < max_color_val + 1) or !iswspace (sep))
    return EXIT_FAILURE;
  ...
  if (fmt == "P6")
    {
      src.read (reinterpret_cast<char *> (img.data ().begin ()),
                img.width () * img.height () * sizeof (color<compT>));
    }
  else
    {
      size_t r, g, b;
      for (auto i : vs::iota (0UL, img.width ()))
        for (auto j : vs::iota (0UL, img.height ()))
          {
            src >> r >> g >> b;
            img (i, j) = { static_cast<compT> (r), static_cast<compT> (g),
                           static_cast<compT> (b) };
          }
    }
...
template <fmt fmt, typename compT = uint8_t>
int
dump (std::ostream &dst, const canvas<compT> &img)
{
  if (!dst.good ())
    return EXIT_FAILURE;

  const size_t max_color_value = (1 << 8 * sizeof (compT)) - 1;
  ...
  if constexpr (fmt == fmt::P3)
    ...
    {
      dst.write (reinterpret_cast<const char *> (img.data ().begin ()),
                 img.width () * img.height () * sizeof (color<compT>));
    }
